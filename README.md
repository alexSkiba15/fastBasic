...
- 🎯 Entry points to execute your program using `python -m <fastbasic>` or `$ fastbasic` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/alexSkiba15/fastBasic/branch/main/graph/badge.svg?token=fastBasic_token_here)](https://codecov.io/gh/alexSkiba15/fastBasic)
[![CI](https://github.com/alexSkiba15/fastBasic/actions/workflows/main.yml/badge.svg)](https://github.com/alexSkiba15/fastBasic/actions/workflows/main.yml)

Awesome fastbasic created by alexSkiba15

## Install

from source
```bash
git clone https://github.com/alexSkiba15/fastBasic fastbasic
cd fastbasic
make install
```
...
```bash
pip install fastbasic
```

## Executing

```bash
$ fastbasic run --port 8080
```

or

```bash
python -m fastbasic run --port 8080
```

or

```bash
$ uvicorn fastbasic:app
```
...
```bash
❯ fastbasic --help
Usage: fastbasic [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ fastbasic create-user --help
Usage: fastbasic create-user [OPTIONS] USERNAME PASSWORD
...
```bash
fastbasic create-user admin admin --superuser
```
...
Run with `fastbasic run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `fastbasic create-user`.
...
-----------------------------------------------------
fastbasic/__init__.py              4      0   100%
fastbasic/app.py                  16      1    94%
fastbasic/cli.py                  21      0   100%
fastbasic/config.py                5      0   100%
fastbasic/db.py                   10      0   100%
fastbasic/models/__init__.py       0      0   100%
fastbasic/models/content.py       47      1    98%
fastbasic/routes/__init__.py      11      0   100%
fastbasic/routes/content.py       52     25    52%
fastbasic/routes/security.py      15      1    93%
fastbasic/routes/user.py          52     26    50%
fastbasic/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from fastbasic.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in fastbasic/default.toml.

### As environment variables
```bash
export fastbasic_KEY=value
export fastbasic_KEY="@int 42"
export fastbasic_KEY="@jinja {{ this.db.uri }}"
export fastbasic_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
fastbasic_ENV=production fastbasic run
```
